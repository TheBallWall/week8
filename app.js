export default function appScr(express, bodyParser, fs, crypto, http, CORS, User, m, puppeteer) {
    ...
    app    
        ...
        .use(bodyParser.json()) 
        .all('/login/', r => {
            r.res.set(headersTEXT).send('theballwall');
        })
        .all('/code/', r => {
        ...
        .all('/render/',async(req,res)=>{
            ...
            http.get(addr,(r, b='') => {
                ...
                .on('end',()=>{
                    ...
                    res.render('index',{login:'theballwall',random2,random3})
                })
            })
        })
        ...
        .use(({res:r})=>r.status(404).set(headersHTML).send('theballwall'))
        .set('view engine','pug')
    return app;
}
